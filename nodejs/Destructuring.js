...
const {log:l , error , table , log:log} = console;
const {exit:E, exit:exit} = process;
...
let { name, sex, age} = fn_ret_object();
let { name:nm, sex:gendor } = fn_ret_object();
...
// 参数是一组无次序的值
function fn_argv_use_obj({x, y, z}) {
    l(x, y, z)
}
fn_argv_use_obj({z: 3, y: 2, x: 1});
...
function fn_set_default_val({name= 'noname', age= 10, sex= 'male'}){
    l(name , age , sex)
}
fn_set_default_val({});
...
// 场景七：输入模块的指定方法
const { add, dec , show} = require("./lib/submodule");
